...
const ViewMail = (props) => {
  ...
  const deleteMailHandler = async () => {
    ...
    if (props.type === "recevied") {
      url = `https://mailbox-client-2ab38-default-rtdb.firebaseio.com/rec${props.email}/${props.mail.id}.json`;
      dispatch(mailActions.deleteReceivedMail(props.mail.id));
    } else {
      url = `https://mailbox-client-2ab38-default-rtdb.firebaseio.com/sent${props.email}/${props.mail.id}.json`;
